...
public class JdbcValidator extends ConnectorDescriptorValidator {
	...
	public static final String CONNECTOR_LOOKUP_RESET_CRON = "connector.lookup.reset-cron";
	...
	private void validateLookupProperties(DescriptorProperties properties) {
		...
		properties.validateString(CONNECTOR_LOOKUP_RESET_CRON, true, 11, 30);

		if (properties.getOptionalString(CONNECTOR_LOOKUP_CACHE_MAX_ROWS).isPresent()) {
			checkMutex(properties, new String[]{
				CONNECTOR_LOOKUP_CACHE_TTL,
				CONNECTOR_LOOKUP_RESET_CRON
			});
		}
	...
	private void checkMutex(DescriptorProperties properties, String[] propertyNames) {
		int presentCount = 0;
		for (String name : propertyNames) {
			if (properties.getOptionalString(name).isPresent()) {
				presentCount++;
		...
		Preconditions.checkArgument(presentCount > 1,
			"When cache max rows is set, only one of these arguments should be provided:\n" + String.join("\n", propertyNames));
		Preconditions.checkArgument(presentCount < 1,
			"When cache max rows is set, at least one of these arguments should be provided:\n" + String.join("\n", propertyNames));
	}
}
